...
Servant is a collection of libraries used for web programming (created by Haskell devs in Singapore). The 'core' servant package is 'servant', and all other packages depend on servant to implement their functionality.

Servant can be described as an 'embedded-DSL'. This just means it isn't a full language (i.e. like Haskell / Purescript), but a domain-specific one. "Embedded" means this langauge exists inside the host language (Haskell in this case). Furthermore, it is embedded at the type-level. This might seem like an odd choice because there is not as much (albeit still a lot) expressive power at the type-level, as the value level. Haskell is not the best language for type-level programming (see Idris, Agda, Coq), but Haskell is the most practical functional programming language for real-world work, and therefore learning the type-level facilities it provides to us can be very beneficial for day-to-day work. In summary, servant is an embedded domain specific langauge, where the domain is Web programming.
...
-- | Capture, allows variable substitution in a path fragment
-- GET /api/dog/{name}
-- Capture "
...
>
> larry, susan :: Person
> larry = Person "Larry" 55
> susan = Person "Susan" 55
>
> type PostAPI =
>   "post"
>     :> ReqBody '[JSON] Person
...
> getEcho :: Maybe String -> Maybe String -> IO (Either Error Value)
...
-- limited language with terms that are used to describe a particular domain, along with a way to interpret this language into objects that suit the domain. Servant is one such domain specific language for web programming

-- We discussed how servant is similar in spirit to GHC.Generics, where GHC.Generics can be thought of as a domain specific language for reifying objects from Haskell records. We showed how we can use the same type-level induction and reification techniques to extract information from a Haskell record's metadata at compile time.

-- We discussed various type-level extensions like PolyKinds, DataKinds, TypeOperators (all of which servant would not be possible without). We overviewied type level symbols and naturals that are enabled via the DataKinds extension.

-- We showed the inner workings of a real servant interpretation for a subset of the servant grammar that actually works on a real world API.

-- After this talk you should be more equipped to deal with type level programming in GHC and be an informed user of the Haskell servant library.
